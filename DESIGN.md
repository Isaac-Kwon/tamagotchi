...
- 리포트 시각 체크(`report.is_due`)는 두 모드 공통으로 스텝 사이마다 이루어지고,
  ...
  맞습니다.
- **stale 판정은 PLAN("updated_at이 기준 간격 2배 초과 시 stale")에서 달라졌습니다.**
  `state.json`은 스텝이 **끝날 때만** 쓰이는데 위의 장기 스텝 정책상 스텝은 주기를
  훌쩍 넘길 수 있어, 원래 규칙은 continuous 모드(기준 60초)에서 정상 진행 중인
  스텝을 거의 항상 stale로 오판했습니다. 현재 규칙(`soul/web/api.py:stale_deadline`):
  `max(updated_at + 기준 간격, next_wake_at) + step_timeout_minutes`까지 쓰기가
  없어야 stale — 즉 "살아 있는 루프라면 하드 데드라인까지는 반드시 뭔가 썼어야 할
  시각"을 넘겼을 때만 죽었다고 판단합니다. 같은 기준 시각을 `stale_at`으로
  `/api/state`에 내려보내, SSE가 조용해진 뒤에도 클라이언트가 자기 시계로 stale을
  뒤집을 수 있습니다(죽은 루프는 stale=true를 실어 보낼 이벤트 자체를 만들지
  못하므로).
