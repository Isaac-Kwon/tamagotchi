...
| GET | `/api/config` | Read-only display config for the settings popover, grouped by section (`llm`, `agent`, `chat`, `sandbox`, `skills`, `web_actions`, `knowledge`, `observer_requests`, `report`, `web`) — safe fields only. Never exposes secrets/deployment internals (`llm.api_key`, `llm.api_key_env`, `agent.data_dir`, `web.host`, `web.port`, `web.allowed_networks`). |
